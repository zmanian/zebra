...
impl EphemeralPublicKey {
    ...
    ///
    /// Returns `None` if the bytes do not represent a valid Jubjub point.
    pub fn inner(&self) -> Option<jubjub::AffinePoint> {
        jubjub::AffinePoint::from_bytes(self.0).into_option()
    }
}
...
impl ValidatingKey {
    ...
    ///
    /// Returns `None` if the bytes do not represent a valid verification key.
    pub fn inner(&self) -> Option<redjubjub::VerificationKey<SpendAuth>> {
        redjubjub::VerificationKey::<SpendAuth>::try_from(self.0).ok()
    }
}
...
impl TryFrom<ValidatingKey> for redjubjub::VerificationKey<SpendAuth> {
    ...
    fn try_from(rk: ValidatingKey) -> Result<Self, Self::Error> {
        rk.inner()
            .ok_or("Invalid redjubjub::VerificationKey for Sapling ValidatingKey")
    }
}
