...
| Nondeterministic faulty message injection in `Init` | `InitWithFaultyEvidence`, `CrosslinkBaselineFaultyInitTinyModel`, `CrosslinkBaselineFaultyInitForkingModel`, `CrosslinkBaselineFullFaultyInitForkingModel` | Partial | Covered in a tiny full-powerset harness, a bounded symbolic forking harness, and a full-powerset quick-check forking `n4_f1` harness; not lifted into all larger parameterized instances. |
...
| Proposal handling in propose step | `UponProposalPrevote` | Partial | Covers proposal-to-prevote behavior with freshness and lock checks. It does not fully port the upstream split between proposal-only and proposal-plus-prevote-valid-round handlers. |
| Upstream valid-round proposal path | `validValue`, `validRound`, `StickyOrStreamProposal`, `UponValuePrevoteQuorum` | Partial | Baseline preserves lock/valid state, but the complete upstream `validRound` proof surface is not yet ported. |
...
| Faulty init symbolic checking | Tiny full-powerset faulty init plus bounded forking faulty init; full forking faulty init remains quick-check-only | Partial |
...
3. Make faulty proposal, prevote, and precommit injection usable in more larger
   parameterized instances, and find a tractable symbolic shape beyond the tiny
   and bounded harnesses. The fixed-sigma/forking `n4_f1` instance now has a
   full-powerset quick check.
