...
| Any precommit quorum handling | `StartNextRoundAfterPrecommitQuorum`, `Decide`, `ApplyLateNilPrecommitCertificate`, `BaselineStartNextRoundAfterPrecommitQuorum`, `BaselineApplyLateNilPrecommitCertificate`, `BaselineDecide`, `BaselineUponPrecommitQuorumEvidence`, `parameterizedShellLateNilCertificateAliasStaysDisabledTest`, `parameterizedShellPrecommitQuorumEvidenceCarriesQuorumTest`, `parameterizedShellPrecommitQuorumEvidenceDispatchesNilRoundAdvanceTest`, `parameterizedShellPrecommitQuorumEvidenceRejectsSubThresholdTest` | Covered | `BaselineUponPrecommitQuorumEvidence(p, r, evidence)` is the upstream evidence-set-parameter shape; it ranges over arbitrary precommit-evidence subsets of size >= `THRESHOLD2` and dispatches between the value-commit branch and the sticky round-advance branch while preserving sticky baseline semantics. |
...
| Round catchup | `RoundCatchupEvidence`, `CatchUpToRound`, `BaselineRoundCatchupEvidence`, `BaselineCatchUpToRound` | Covered | Catchup requires `T + 1` observed activity in the target round. Shell action now drives `BaselineNext` directly. |
| System transition `Next` | `CrosslinkResampling.qnt` `Next` (focused model, retained as reference); `CrosslinkBaselineTenderlink.qnt` `BaselineNext` as first-class `any { ... }` transition system | Covered | `BaselineNext` no longer delegates to the focused `Next`; it inlines the upstream `Tendermint.qnt` `Next` shape with baseline-prefixed shell actions. Crosslink rules (`head - sigma` value selection, sticky nil-precommit, decision freshness) are preserved by construction. `BaselineUponPrecommitQuorumEvidence` matches the upstream evidence-set parameter shape. |
...
| Faulty init symbolic checking | Tiny full-powerset faulty init plus bounded forking faulty init for `n4_f1`, representative `n4_f2`/`n5_f2`/`n7_f2`, single-/pair-/triple-faulty full-domain `n4_f2` abstractions, single-/pair-/triple-faulty full-domain `n5_f2` abstractions, and single-/pair-/triple-faulty full-domain `n7_f2`; full forking faulty init remains quick-check-only for `n4_f1`, `n4_f2`, `n5_f1`, `n5_f2`, and `n7_f2` | Partial |
...
2. Broaden the tractable symbolic shape for faulty proposal, prevote, and
   ...
   bounded `n4_f1`, representative `n4_f2`/`n5_f2`/`n7_f2`,
   single-/pair-/triple-faulty full-domain `n4_f2` harnesses,
   single-/pair-/triple-faulty full-domain `n5_f2` harnesses, and the
   single-/pair-/triple-faulty full-domain `n7_f2` harnesses. The
