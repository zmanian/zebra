...
- `CrosslinkBaselineAccountability.qnt`
  ...
  - adds quick-check-only fixed-sigma/forking faulty-init harnesses for `n4_f1`,
    `n5_f1`, and `n7_f2` using the full faulty proposal, prevote, and precommit
    powerset domains
...
3. Add upstream-style faulty message injection.
   ...
   - The full faulty-init powerset is now exercised for the fixed-sigma/forking
     `n4_f1`, `n5_f1`, and `n7_f2` instances in `quick-baseline`; remaining
     work is broader instances and symbolic coverage that stays tractable.
...
1. Add upstream-style faulty message injection to the parameterized shell.
   ...
   - Quick-check-only fixed-sigma/forking `n4_f1`, `n5_f1`, and `n7_f2`
     harnesses now cover the full faulty-evidence domain. The missing piece is
     extending that coverage to the remaining parameterized instances and
     finding a tractable symbolic abstraction for more than the bounded domain.
