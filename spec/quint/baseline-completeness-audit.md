...
| Parameterized `Corr/Faulty/N/T` validator model | `CrosslinkBaselineTenderlink.qnt`; `BaselineInitWithFaultyEvidence`; `BaselineStartRound`; `BaselineNext`; `BaselineFaultyInitSafety`; `CrosslinkBaselineParameterizedShellTest` | Partial; parameter shell now exposes the full faulty-init evidence surface, StartRound-style round initialization, and transition step through baseline-prefixed aliases, while the full transition surface is still focused |
...
3. Add upstream-style faulty message injection.
   ...
   - The full faulty-init powerset is now exercised for the fixed-sigma/forking
     ...
     `quick-baseline`.
   - The single-faulty `n4_f2` harness symbolically ranges over one arbitrary
     faulty proposal, prevote, and precommit from the full `n4_f2` domain at
     max depth 2; remaining work is broader symbolic coverage that stays
