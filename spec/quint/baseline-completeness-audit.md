...
- `CrosslinkBaselineAccountability.qnt`
  ...
  - adds a quick-check-only fixed-sigma/forking `n4_f1` faulty-init harness
    using the full faulty proposal, prevote, and precommit powerset domains
...
| Nondeterministic faulty message injection in `Init` | `InitWithFaultyEvidence`; `CrosslinkBaselineFaultyInitTinyModel`; `CrosslinkBaselineFaultyInitForkingModel`; `CrosslinkBaselineFullFaultyInitForkingModel`; `BaselineFaultyInitSafety`; `BaselineForkingFaultyInitSafety`; `BaselineFullForkingFaultyInitSafety` | Partial; covered by a tiny full-powerset instance, a bounded symbolic fixed-sigma/forking instance, and a quick-check full-powerset fixed-sigma/forking `n4_f1` instance; not yet lifted into every larger parameterized instance or symbolic gate |
...
3. Add upstream-style faulty message injection.
   ...
   - The full faulty-init powerset is now exercised for the fixed-sigma/forking
     `n4_f1` instance in `quick-baseline`; remaining work is broader instances
     and symbolic coverage that stays tractable.
...
1. Add upstream-style faulty message injection to the parameterized shell.
   ...
   - A quick-check-only fixed-sigma/forking `n4_f1` harness now covers the full
     faulty-evidence domain. The missing piece is extending that coverage to
     more parameterized instances and finding a tractable symbolic abstraction
     for more than the bounded domain.
