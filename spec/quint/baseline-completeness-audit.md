...
- `CrosslinkBaselineAccountability.qnt`
  ...
  - adds a single-faulty `n7_f2` symbolic harness that selects one arbitrary
    faulty proposal, prevote, and precommit from the larger full `n7_f2`
    faulty evidence domains
...
1. Broaden upstream-style faulty message injection beyond the current shell
   ...
   - A single-faulty `n7_f2` harness now ranges over the proper f=2 boundary's
     full faulty-evidence domain by selecting one arbitrary faulty proposal,
     prevote, and precommit at max depth 2.
