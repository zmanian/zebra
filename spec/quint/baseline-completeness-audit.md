...
- `CrosslinkBaselineAccountability.qnt`
  ...
  - adds quick-check-only fixed-sigma/forking faulty-init harnesses for `n4_f1`
    and `n7_f2` using the full faulty proposal, prevote, and precommit powerset
    domains
...
| Nondeterministic faulty message injection in `Init` | `InitWithFaultyEvidence`; `CrosslinkBaselineFaultyInitTinyModel`; `CrosslinkBaselineFaultyInitForkingModel`; `CrosslinkBaselineFullFaultyInitForkingModel`; `CrosslinkBaselineFullFaultyInitN7F2ForkingModel`; `BaselineFaultyInitSafety`; `BaselineForkingFaultyInitSafety`; `BaselineFullForkingFaultyInitSafety`; `BaselineFullN7F2ForkingFaultyInitSafety` | Partial; covered by a tiny full-powerset instance, a bounded symbolic fixed-sigma/forking instance, and quick-check full-powerset fixed-sigma/forking `n4_f1` and `n7_f2` instances; not yet lifted into every larger parameterized instance or symbolic gate |
...
| Generated/adversarial PoW schedule for baseline long reorgs | `CrosslinkBaselinePowSampling.qnt`; `CrosslinkBaselinePowSamplingModel`; `CrosslinkBaselinePowLongReorgModel`; `CrosslinkBaselinePowGeneratedScheduleModel`; `BaselinePowSamplingSafety`; `BaselinePowLongReorgSafety`; `BaselinePowGeneratedScheduleSafety` | Partial; bounded fork-switch, long-reorg, and generated adversarial work-competition fixtures are covered, but stochastic repeated stream-change schedules remain open |
...
3. Add upstream-style faulty message injection.
   ...
   - The full faulty-init powerset is now exercised for the fixed-sigma/forking
     `n4_f1` and `n7_f2` instances in `quick-baseline`; remaining work is
     broader instances and symbolic coverage that stays tractable.
...
7. Expand the PoW environment.
   ...
   - A long-reorg fixture now covers rollback depth 3 with sigma 2, where the
     ...
     switch.
   - Add a simple stochastic block-production abstraction, and broaden the
     generated schedule to repeated stream changes rather than one adversarial
     release.
...
1. Add upstream-style faulty message injection to the parameterized shell.
   ...
   - Quick-check-only fixed-sigma/forking `n4_f1` and `n7_f2` harnesses now
     cover the full faulty-evidence domain. The missing piece is extending that
     coverage to the remaining parameterized instances and finding a tractable
     symbolic abstraction for more than the bounded domain.
