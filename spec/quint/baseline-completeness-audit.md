...
- `CrosslinkBaselineAccountability.qnt`
  ...
  - adds a single-faulty `n4_f2` symbolic harness that selects one arbitrary
    faulty proposal, prevote, and precommit from the full `n4_f2` faulty
    evidence domains
...
3. Add upstream-style faulty message injection.
   ...
   - The full faulty-init powerset is now exercised for the fixed-sigma/forking
     ...
     `quick-baseline`.
   - The single- and pair-faulty `n4_f2` harnesses symbolically range over one
     or two arbitrary faulty proposals, prevotes, and precommits from the full
     `n4_f2` domain at max depth 2; remaining work is broader symbolic
     coverage that stays tractable. A local Apalache probe of
...
1. Broaden upstream-style faulty message injection beyond the current shell
   ...
   - Single- and pair-faulty `n4_f2` harnesses now symbolically range over the
